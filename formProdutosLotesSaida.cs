...
{
    public partial class formProdutosLotesSaida : Form
    {
        ...
        private void buttonAdicionar_Click(object sender, EventArgs e)
        {
            ...
            {
                if (quantidade_no_sistema == 0)
                {
                    MessageBox.Show("O sistema detectou que não temos esse produto na localização informada!", "Atenção!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    MessageBox.Show("O sistema detectou que só temos " + quantidade_no_sistema + " unidades desse produto na localização informada!", "Atenção!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        ...
        private void AtualizarQuantidadeAtual()
        {
            try
            {
                quantidade_no_sistema = comandos.QuantidadeDeProdutosComOCodigoDeBarras(cod_barras, prateleira) - Produtos.Where(x => x.Cod_Barras == cod_barras).Sum(x => x.Quantidade);
        ...
        private void AtualizarDataGrid()
        {
            ...
            var lista = Produtos.GroupBy(x => (x.Cod_Barras, x.Nome)).Select(g => (g.Key.Cod_Barras, g.Key.Nome, Total: g.Sum(x => x.Quantidade)));

            foreach (var produto in lista)
            {
                dataGridViewLista.Rows.Add(produto.Cod_Barras, produto.Nome, produto.Total);
            }
        ...
        private void buttonSalvar_Click(object sender, EventArgs e)
        {
            ...
            if (total == 0)
            {
                MessageBox.Show("Informe ao menos um produto para continuar!", "Atenção!", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                comandos.ApagarProdutosDaTabelaDeLotes(Produtos);
                Dispose();
            }
        }

        private void formProdutosLotesSaida_Load(object sender, EventArgs e)
        {
        ...
        private void formProdutosLotesSaida_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void comboBoxLocalizacoes_SelectedIndexChanged(object sender, EventArgs e)
        {
            prateleira = comboBoxLocalizacoes.Text;
            AtualizarQuantidadeAtual();
        }
